...
module.exports = CalcBox = (function() {
  ...
  CalcBox.prototype.pack = function() {
    var i, len, origBox, p, parcels, ref, ref1;
    ...
    parcels.sort(function(a, b) {
      ...
      if (v = b.width * b.height * b.depth - a.width * a.height * a.depth) {
        return v;
      }
      return Math.min(b.width, b.height, b.depth) - Math.min(a.width, a.height, a.depth);
    });
    if ((function(_this) {
      return function() {
        var i, len, p;
        for (i = 0, len = parcels.length; i < len; i++) {
          p = parcels[i];
    ...
    parcels.sort(function(a, b) {
      ...
      if (v = a.width * a.height * a.depth - b.width * b.height * b.depth) {
        return v;
      }
      return Math.max(a.width, a.height, a.depth) - Math.max(b.width, b.height, b.depth);
    });
    for (i = 0, len = parcels.length; i < len; i++) {
      p = parcels[i];
